...
adv <- info
info <- subset(info, AJCC_PATHOLOGIC_TUMOR_STAGE %in% c('STAGE I','STAGE IA','STAGE II','STAGE IIA','STAGE IIB','STAGE IIC'))
write.table(info, file='TCGA_COADREAD_EarlyStage_MSS_sampleID.txt',row.names=F,quote=F,sep='\t')
adv <- subset(adv, AJCC_PATHOLOGIC_TUMOR_STAGE %in% c('STAGE III','STAGE IIIA','STAGE IIIB','STAGE IIIC','STAGE IV','STAGE IVA','STAGE IVB'))
write.table(adv, file='TCGA_COADREAD_AdvancedStage_MSS_sampleID.txt',row.names=F,quote=F,sep='\t')
dat <- subset(info, DSS_STATUS!='1:DEAD WITH TUMOR' & DFS_STATUS!='1:Recurred/Progressed' & PFS_STATUS!='1:PROGRESSION')
dat1 <- subset(dat, DSS_STATUS=='0:ALIVE OR DEAD TUMOR FREE' | DFS_STATUS=='0:DiseaseFree' | PFS_STATUS=='0:CENSORED')
dat1$PFS_MONTHS <- as.numeric(dat1$PFS_MONTHS)
dat2 <- subset(dat1, PFS_MONTHS>=24)
write.table(dat2, file='TCGA_COADREAD_EarlyStage_MSS_2yr.follow.up_sampleID.txt',row.names=F,quote=F,sep='\t')
setwd('<PROJECT_ROOT>')
options(stringsAsFactors=F)
library(data.table)
rm(list=ls())
...
max(genmut[,2:ncol(genmut)])
filename <- c('TCGA_COADREAD_EarlyStage_MSS',
'TCGA_COADREAD_EarlyStage_MSS_2yr.follow.up')
for(grp in filename){
